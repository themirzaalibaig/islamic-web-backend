...
export const createTasbeehSchema = z.object({
  ...
  target: z.number().int('Target must be an integer').min(1, 'Target must be at least 1').max(1000000, 'Target cannot exceed 1000000'),
});
...
export const updateTasbeehSchema = z.object({
  ...
  text: z.string().min(1, 'Text is required').max(500, 'Text cannot exceed 500 characters').trim().optional(),
  target: z.number().int('Target must be an integer').min(1, 'Target must be at least 1').max(1000000, 'Target cannot exceed 1000000').optional(),
